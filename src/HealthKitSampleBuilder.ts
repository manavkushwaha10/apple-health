...
/**
 * Create a builder for quantity samples.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export function buildQuantitySample(type: QuantityTypeIdentifier): HealthKitSampleBuilder {
  return new HealthKitSampleBuilder().quantityType(type);
}

/**
 * Create a builder for category samples.
 *
 * @example
 * ```ts
 * const sample = await buildCategorySample('sleepAnalysis')
 *   .categoryValue(1)
 *   .startDate(bedtime)
 *   .endDate(wakeUp)
 *   .save();
 * ```
 */
export function buildCategorySample(type: CategoryTypeIdentifier): HealthKitSampleBuilder {
  return new HealthKitSampleBuilder().categoryType(type);
}

/**
 * Create a builder for workout samples.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export function buildWorkout(activityType: WorkoutActivityType): HealthKitSampleBuilder {
  return new HealthKitSampleBuilder().workoutType(activityType);
}
